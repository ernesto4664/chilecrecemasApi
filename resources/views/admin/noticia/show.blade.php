    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <p><strong>Imagen:</strong> <img src="{{ $noticia->imagen }}" alt="Imagen de la noticia" style="max-width: 300px;"></p>
                <p><strong>Fecha y Hora:</strong> {{ $noticia->fecha_hora }}</p>
                <p><strong>Status:</strong> {{ $noticia->status }}</p>
                <p><strong>Prioridad:</strong> {{ $noticia->privilegio }}</p>
                <p><strong>Tags:</strong> {{ $noticia->tag->nombre }}</p>
                <p><strong>Usuario:</strong> {{ $noticia->usuariop_id }}</p>
            </div>
        </div>
        <a href="{{ route('noticias.index') }}" class="btn btn-primary mt-3">Regresar</a>
    </div>
@endsection
